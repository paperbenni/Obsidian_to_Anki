...
export class FileManager {
  ...
  findFilesThatAreNotIgnored(files: TFile[], data: ParsedSettings): TFile[] {
    ...
    let notIgnoredFiles = files.filter(
      (file) => !ignoredFiles.includes(file.path)
    );
  ...
  async genAllFiles() {
    for (let file of this.files) {
      ...
      const cache: CachedMetadata | null = this.app.metadataCache.getCache(
        file.path
      );
      if (cache) {
  ...
  async parse_requests_1() {
    ...
    for (let index in note_info_array_by_file) {
      let i: number = parseInt(index);
      let file = this.ownFiles[i];
      const file_response = AnkiConnect.parse(
        note_info_array_by_file[i] as any
