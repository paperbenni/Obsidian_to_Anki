...
export class FormatConverter {
    ...
    formatCloze(str: string): string {
        return str.replace(c.OBS_CLOZE_REGEXP, (match, pre, cloze_content, post) => {
            return (pre || "") + (cloze_content || "").replace(/<br>/g, " ") + (post || "")
        }).replace(c.OBS_CLOZE_HIGHLIGHT_REGEXP, (match, pre, cloze_content, post) => {
            return (pre || "") + "{{c1::" + (cloze_content || "") + "}}" + (post || "")
        })
    }

    formatHighlights(str: string): string {
        return str.replace(c.OBS_HIGHLIGHT_REGEXP, (match, pre, content, post) => {
            return (pre || "") + "<mark>" + (content || "") + "</mark>" + (post || "")
        })
    }
    ...
    formatLinks(str: string): string {
        return str.replace(c.OBS_WIKILINK_REGEXP, (match, text, post) => {
            ...
            const link = "obsidian://open?vault=" + this.vault_name + "&file=" + parsed.path
    ...
    formatMedia(str: string): string {
        ...
        return str.replace(c.OBS_EMBED_REGEXP, (match, pre, alt, link, post) => {
            ...
            return (pre || "") + '<img alt="' + alt + '" src="' + media_name + '" />' + (post || "")
        })
    }
    ...
    formatMath(str: string): string {
        ...
        str = str.replace(c.OBS_INLINE_MATH_REGEXP, (match, pre, expression, post) => {
            return (pre || "") + "\\(" + (expression || "") + "\\)" + (post || "")
        })
        str = str.replace(c.OBS_DISPLAY_MATH_REGEXP, (match, pre, expression, post) => {
            return (pre || "") + "\\[" + (expression || "") + "\\]" + (post || "")
        })
        return str
    }
    ...
    formatCode(str: string): string {
        ...
        str = str.replace(c.OBS_CODE_REGEXP, (match, pre, expression, post) => {
            return (pre || "") + "<code>" + (expression || "").replace(/<br>/g, "\n") + "</code>" + (post || "")
        })
        str = str.replace(c.OBS_DISPLAY_CODE_REGEXP, (match, pre, lang, expression, post) => {
            const highlight_class = lang ? `class="language-${lang}"` : ""
            return (pre || "") + "<pre><code " + highlight_class + ">" + (expression || "").replace(/<br>/g, "\n") + "</code></pre>" + (post || "")
        })
        return str
    }
