...
export const DEFAULT_IGNORED_FILE_GLOBS: string[] = ["**/templates/**"];
...
export class SettingsTab extends PluginSettingTab {
  ...
  sanitise_input(
    key: keyof PluginSettings["Defaults"],
    input: string
        ...
        .addText((text) =>
          text
            .setPlaceholder(
              String(
                this.plugin.settings.Defaults[
                  key as keyof PluginSettings["Defaults"]
                ]
              )
            )
            .setValue(
            ...
            .onChange(async (value) => {
              const [valid, sanitised] = this.sanitise_input(
                key as keyof PluginSettings["Defaults"],
                value
              );
              if (valid) {
      ...
      for (let note_type of this.note_types) {
        new Setting(containerEl)
          ...
          .addDropdown((dropdown) => {
            ...
            if (this.plugin.settings.FILE_LINK_FIELDS) {
              dropdown.setValue(
                this.plugin.settings.FILE_LINK_FIELDS[note_type]
              );
            }
