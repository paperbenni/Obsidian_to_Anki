...
export const OBS_WIKILINK_REGEXP: RegExp = new RegExp(OBS_WIKILINK_REGEXP_STR, 'g')
...
export const OBS_INLINE_MATH_REGEXP: RegExp = new RegExp(OBS_INLINE_MATH_REGEXP_STR, 'g')

export const OBS_DISPLAY_MATH_REGEXP_STR: string = String.raw`(?<pre>[^$]|^)\$\$([^$\n]+?)\$\$(?<post>[^$]|$)`
export const OBS_DISPLAY_MATH_REGEXP: RegExp = new RegExp(OBS_DISPLAY_MATH_REGEXP_STR, 'g')
...
export const OBS_DISPLAY_CODE_REGEXP: RegExp = new RegExp(OBS_DISPLAY_CODE_REGEXP_STR, 'g')

export const CODE_CSS_URL: string = "https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.5.1/styles/default.min.css"

export const OBS_CLOZE_REGEXP: RegExp = /(?:(?<!{){(?:c?(\d+)[:|])?(?!{))((?:[^\n][\n]?)+?)(?:(?<!})}(?!}))/g
