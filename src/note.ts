...
const ID_REGEXP_STR: string = String.raw`(?:\s*<!--ID: (\d+)-->\s*$)`
...
export class Note {
    ...
    constructor(note: string, fields_dict: Record<string, string[]>, curly_cloze: boolean, highlights_to_cloze: boolean, formatter: FormatConverter) {
        this.note = note
...
export class InlineNote extends Note {
    ...
    getFields(): Record<string, string> {
        ...
        if (this.fields_dict[this.note_type] && lines.length != this.fields_dict[this.note_type].length) {
            this.note_type = ""
            return {}
        }
        if (this.fields_dict[this.note_type]) {
            for (let i in lines) {
                fields[this.fields_dict[this.note_type][i]] = this.formatter.format(lines[i])
            }
        }
        return fields
    }
...
export class RegexNote extends Note {
    ...
    constructor(
        match: RegExpMatchArray, note_type: string, fields_dict: Record<string, string[]>,
        search_tags: boolean, search_id: boolean, curly_cloze: boolean,
        highlights_to_cloze: boolean, formatter: FormatConverter
    ) {
