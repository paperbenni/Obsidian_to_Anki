...
function spans(pattern: RegExp, text: string): Array<[number, number]> {
  ...
  for (let match of matches) {
    output.push([match.index || 0, (match.index || 0) + match[0].length]);
  }
  return output;
}
  ...
  add_spans_to_ignore() {
    ...
    if (deck_result) {
      this.ignore_spans.push([
        deck_result.index || 0,
        (deck_result.index || 0) + deck_result[0].length,
      ]);
    }
    ...
    if (tag_result) {
      this.ignore_spans.push([
        tag_result.index || 0,
        (tag_result.index || 0) + tag_result[0].length,
      ]);
    }
    ...
    for (let note_match of this.file.matchAll(this.data.NOTE_REGEXP)) {
      ...
      let parsed = new Note(
        ...
        this.data.add_context
          ? this.getContextAtIndex(note_match.index || 0)
          : ""
      );
      ...
      for (let search_tags of [true, false]) {
        ...
        for (let match of findignore(regexp, this.file, this.ignore_spans)) {
          ...
          const parsed: AnkiConnectNoteAndID = new RegexNote(
            ...
            this.data as any,
            this.data.add_context
              ? this.getContextAtIndex(match.index || 0)
              : ""
          );
          if (search_id) {
            if (
